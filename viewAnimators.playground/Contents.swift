...
extension AnimationViewController {
    ...
    func bounceKeyFrame(withDuration duration: TimeInterval, delay: TimeInterval = 0) {
        view.layoutIfNeeded()
        let animation = UIViewPropertyAnimator(duration: duration, curve: .linear)
        ...
        animation.addAnimations {
            ...
            UIView.animateKeyframes(withDuration: duration, delay: delay, animations: {
                ...
                UIView.addKeyframe(withRelativeStartTime: 0.0, relativeDuration: 0.5) {
                    self.redSquare.bounds.size = CGSize(width: 300, height: 300)
                    self.view.layoutIfNeeded()
                }
                
                UIView.addKeyframe(withRelativeStartTime: 0.5, relativeDuration: 0.5) {
                    self.redSquare.bounds.size = CGSize(width: 200, height: 200)
                    self.view.layoutIfNeeded()
                }
            })
        }
...
// vc.propertyKeyFrame(withDuration: animationDuration, delay: animationDelay)
 vc.bounceKeyFrame(withDuration: animationDuration)
